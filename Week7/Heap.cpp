...
class Heap {
 private:
  ...
  heapIndex getParentPosition(heapIndex i) {
    ...
    return (heapIndex)std::floor((i - 1) / 2);
  }
 ...
 public:
  ...
  void insert(T element) {
    ...
    heapIndex newIndex = tree.size() - 1;
    heapIndex parentIndex = getParentPosition(newIndex);
    while (newIndex > 0 && tree[parentIndex] > tree[newIndex]) {
      T temp = tree[newIndex];
      tree[newIndex] = tree[parentIndex];
      tree[parentIndex] = temp;
      newIndex = parentIndex;
      parentIndex = getParentPosition(newIndex);
    }
  }
  ...
  // TO BE IMPLEMENTED
  // Recursively find the index of the value in the heap starting from index i
  heapIndex find(T value, heapIndex i) {
    ...
    heapIndex leftSearch = find(value, leftChild);

    if (leftSearch != -1) {
      return leftSearch;
    }

    return find(value, find(value, getRightChildPosition(i)));
  }
  ...
  bool isLeaf(heapIndex i) {
    if (i > getParentPosition(tree.size())) {
      return true;
    }
    return false;
  }
